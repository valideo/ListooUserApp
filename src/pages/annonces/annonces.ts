...
export class AnnoncesPage {
  ...
  loadAnnonces(): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      this.apiProvider.apiGetAllAnnonces().then(
        (data) => {
          this.annonces = data;
          this.annonces.forEach((annonce) => {
            this.apiProvider.apiLoadResto(annonce['idRestoUser']).then(
              (dataResto) => {
                var type = this.getTypeName(dataResto['restoType']);
                var address = dataResto['address'] + ', ' + dataResto['city'];
                var picName = annonce['piUrl'].substring(
                  1,
                  annonce['piUrl'].length - 1
                );
                var qtiteLeft = 0;
                this.apiProvider
                  .apiGetCommandesByAnnonce(annonce['id'])
                  .then((data) => {
                    var nbReserved = 0;
                    var orders: any = data;
                    ...
                    var detailObject = {
                      ...
                      idUserResto: annonce['idRestoUser'],
                      restoName: dataResto['restoName'],
                      typeValue: dataResto['restoType'],
                      restoType: type,
                      address: address,
                      tel: dataResto['tel'],
                      piUrl: picName,
                      startHour: annonce['startHour'],
                      endHour: annonce['endHour'],
                      price: (annonce['price'] * 0.3).toLocaleString('es-CO'),
                      initialPrice: annonce['price'].toLocaleString('es-CO'),
                      qtite: annonce['qtite'],
                      desc: annonce['desc'],
                      qtiteLeft: qtiteLeft,
                    };
                    this.checkFilters(detailObject).then((data) => {
                      if (data == true) annonceDetailTemp.push(detailObject);
                    });
                  });
              },
              (err) => {
  ...
  loadAnnoncesInput(): Promise<any> {
    return new Promise((resolve, reject) => {
      this.apiProvider.apiLoadRestosWithFilter(this.searchString).then(
        (data) => {
          this.annonces = data;
          this.annonces.forEach((dataResto) => {
            this.apiProvider.apiGetAnnonceByResto(dataResto['id']).then(
              (dataAnnonce) => {
                var type = this.getTypeName(dataResto['restoType']);
                var address = dataResto['address'] + ', ' + dataResto['city'];
                ...
                var qtiteLeft = 0;
                this.apiProvider
                  .apiGetCommandesByAnnonce(dataAnnonce['id'])
                  .then(
                    (data) => {
                      var nbReserved = 0;
                      var orders: any = data;
                      ...
                      var detailObject = {
                        ...
                        price: (dataAnnonce['price'] * 0.3).toLocaleString(
                          'es-CO'
                        ),
                        initialPrice: dataAnnonce['price'].toLocaleString(
                          'es-CO'
                        ),
                        qtite: dataAnnonce['qtite'],
  ...
  checkFilters(annonceToCheck): Promise<Boolean> {
    ...
    return new Promise((resolve, reject) => {
      this.apiProvider
        .apiGetCommandesByAnnonce(annonceToCheck['idAnnonce'])
        .then((data) => {
          var nbReserved = 0;
          var orders: any = data;
          orders.forEach((element) => {
  ...
  addAlertInputs(alert, value) {
    var isChecked = false;
    if (this.categoriesArray.indexOf(value) >= 0) isChecked = true;
    alert.addInput({
      type: 'checkbox',
  ...
  getTypeName(type: string) {
    ...
    else if (type == 'burger') return 'Hamburguesería';
    else if (type == 'jap') return 'Japonesa';
    else if (type == 'pol') return 'Pollo';
    else if (type == 'col') return 'Colombiana';
    else if (type == 'bowl') return 'Bowl';
    else if (type == 'hot') return 'Hot dog';
    else if (type == 'sushi') return 'Sushi';
    else if (type == 'des') return 'Desayunos';
    else if (type == 'asi') return 'Asiática';
    else if (type == 'mexi') return 'Mexicana';
    else if (type == 'ita') return 'Italiana';
    else if (type == 'ara') return 'Árabe';
    else if (type == 'chu') return 'Chuzo';
    else if (type == 'car') return 'Carne';
    else if (type == 'peru') return 'Peruana';
    else if (type == 'empa') return 'Empanadas';
    else if (type == 'mar') return 'Del mar';
    else if (type == 'sand') return 'Sándwich';
    else if (type == 'jug') return 'Jugos';
    else if (type == 'cafe') return 'Cafès';
    else if (type == 'post') return 'Postres';
  }
