...
export class AnnonceDetailPage {
  ...
  convertValues() {
    this.apiProvider.apiGetCommandesByAnnonce(this.idAnnonce).then(
      (data) => {
        var nbReserved = 0;
        var orders: any = data;
    ...
    var ext = '';
    if (startHourstring.indexOf('p') > 1) ext = 'pm';
    else ext = 'am';
    startHourstring =
      startHourstring.substring(0, startHourstring.length - 9) + ext;

    var endHourString = this.endHour.toLocaleTimeString('es-CO');
    if (endHourString.indexOf('p') > 1) ext = 'pm';
    else ext = 'am';
        ...
        this.apiProvider.apiGetCommandesByAnnonce(this.idAnnonce).then(
          (data) => {
            ...
            if (this.qtiteLeft >= this.qtiteSelected) {
              this.apiProvider
                .apiCreateCommande(this.idAnnonce, this.qtiteSelected, today)
                .then(
                  (data) => {
                    console.log('schedule notif');
                    console.log(
                      new Date(
                        this.startHour.getTime() - 600000
                      ).toLocaleTimeString()
                    );
                    console.log(
                      new Date(
                        this.startHour.getTime() - 5400000
                      ).toLocaleTimeString()
                    );
                    console.log(
                      new Date(this.startHour.getTime()).toLocaleTimeString()
                    );
                    this.localNotifications.schedule([
                      {
                        ...
                        trigger: {
                          at: new Date(this.startHour.getTime() - 5400000),
                        },
                      },
                      {
                        id: 2,
                        ...
                        trigger: {
                          at: new Date(this.startHour.getTime() - 600000),
                        },
                      },
                    ]);
                    ...
                    this.fcm.notifPanierRecup(this.idRestoUser).then(
                      (dataNotif) => {
  ...
  addQtite() {
    if (this.qtiteSelected < this.qtiteLeft) this.qtiteSelected += 1;
  ...
  removeQtite() {
    if (this.qtiteSelected > 1) this.qtiteSelected -= 1;
